...
function ImGui:ApplyDraggable(Frame: Frame, Header: Frame)
	...
	local function Allowed(InputType)
		return table.find(UserInputTypes, InputType)
	end

	Header.InputBegan:Connect(function(Key)
		if Allowed(Key.UserInputType) then
			Dragging = true
			KeyBeganPos = Vector2.new(Key.Position.X, Key.Position.Y)
	...
	local function Movement(Input)
		if not Dragging or not KeyBeganPos then return end
		local CurPos = Vector2.new(Input.Position.X, Input.Position.Y)
		local Delta = CurPos - KeyBeganPos
